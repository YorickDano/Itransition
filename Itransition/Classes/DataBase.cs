...
namespace ItransitionTask3.Classes
{
    ...
    {
        static SqliteConnection connection = new SqliteConnection(@"Data Source=wwwroot\DataBase\Users.db");
        ...
        {
            SqliteCommand command = new SqliteCommand($@"SELECT * FROM Users WHERE Email='{email}' AND Password='{password}' AND Status='UnLock'", connection);
            connection.Open();
            using (SqliteDataReader reader = command.ExecuteReader())
            {
                if (reader.HasRows)
        ...
        public static void ChangeUsersStatus(List<User> users, string status)
        {
            command.Connection = connection;
        ...
        public static void UpdateLastEnterDate(string email)
        {
            connection.Open();
            command.Connection = connection;
        ...
        public void AddInfoInTable(string email, string name, string password)
        {
            string createdDate = DateTime.Now.ToLocalTime().ToString();
            connection.Open();
            command.Connection = connection;
